        ...
        if ($user->can($permission) || $user->user_type == "teacher" || ($user->user_type == "student" && $user->username == $request->student_id)) {
            ...
            if (count($query) > 0) {
                ...
                $marks = $marks->leftJoin("class_has_students", function($join)use ($request) {
                    $join->on("marks.student_id", "=", "class_has_students.id")->where("class_has_students.class_id", "=",$request->class_id)->where("class_has_students.department_id", "=",$request->department_id)->where("class_has_students.session_id", "=",$request->session_id);                    
                });
